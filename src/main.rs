...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let comment_file_header = format!(
        "commentRanking-{}-",
    ...
    let score_sorting_methods_to_use = [
        SortScoringMethod::NormalizedLinear,
        SortScoringMethod::NormalizedPlaceSquared,
    ]; //SortScoringMethod::Linear,SortScoringMethod::PlaceSquared,
    let comment_list_sorting_types = [CommentListSortingMethod::Best]; //, CommentListSortingMethod::Top, CommentListSortingMethod::New];
    let methods_to_handle_low_scoring_members = [
    ...
    for number_of_comments in
        (comment_number_step_size..maximum_number_of_comments).step_by(comment_number_step_size)
    {
        ...
        for index_of_runs in 0..number_of_runs {
            children.push(thread::spawn(move || -> (usize, Vec<(u32, f32)>) {
                let ranking_result =
                    comment_ranking::comment_ranking::simulate_comments_for_one_topic(
                        number_of_comments as u32,
                        maximum_number_of_user_interactions,
                        comment_scoring_method,
                        sorting_method,
                        score_sorting_method,
                        user_interaction_step_size,
        ...
        let mut ranking_average = vec![
            (0, 0.0);
            (maximum_number_of_user_interactions / user_interaction_step_size)
                as usize
        ];
        ...
        for index in 0..ranking_average.len() {
            let one_record = [
                ...
                (ranking_average[index].1 / (ranking_events_to_average as f32)).to_string(), //					number_of_runs.to_string(),
            ];
